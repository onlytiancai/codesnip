...
## "任意两个位置的编码可以通过线性变换表达其相对距离"的含义

通过运行的测试，我们可以清楚地理解这句话：
...
其中 `ω_i = 1/10000^(2i/d_model)` 是频率项。

### 2. **核心洞察：旋转矩阵**
对于任意两个位置 `pos_a` 和 `pos_b`，它们的相对距离是 `Δ = pos_b - pos_a`。

从测试结果可以看到，我们可以构造一个**旋转矩阵**：
```
R(Δ) = [[cos(Δ*ω), -sin(Δ*ω)]
        [sin(Δ*ω),  cos(Δ*ω)]]
```

这个矩阵可以将位置 `pos_a` 的编码对 `[sin(pos_a*ω), cos(pos_a*ω)]` 变换为位置 `pos_b` 的编码对。
...
测试结果显示，手动计算和模型输出的误差为 0.00000000，完全验证了这个数学关系。

### 4. **"线性变换"的含义**
- **线性**：变换是矩阵乘法，满足线性性质
- **相对距离**：相同的相对距离Δ对应相同的变换矩阵
- **位置无关性**：无论在序列的哪个位置，相对距离为3的变换矩阵都是一样的

### 5. **模型学习的好处**
1. **模式识别**：模型可以学会识别特定的相对距离关系
2. **泛化能力**：训练时学到的相对位置关系可以应用到推理时的新位置
3. **位置无关性**：`(1,4)` 和 `(10,13)` 的相对关系是相同的
