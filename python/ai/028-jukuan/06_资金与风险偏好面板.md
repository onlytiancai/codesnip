...
| ERP（股票端） | `get_index_valuation("000985.XSHG")` | `pe_ratio` PE-TTM |
| ERP（债券端） | `bond.REPO_DAILY_PRICE`（GC001） | `close` 短端利率代理，% |
...
```python
# ============================================================
# cell 7: 国债逆回购利率（GC001）作为 10 年国债收益率代理
# ============================================================
...
```python
# ============================================================
# cell 9: 三大模块横向合并（按日期 inner join）
# ============================================================
# inner join：任一指标缺失即丢弃（保守起见保证三模块日期齐全）
df_module5 = (
...
```python
# ============================================================
# cell 14: 图3 - ERP 时序 + 1 年趋势线 + 5 年分位（双 y 轴）
# ============================================================
fig, ax = plt.subplots(figsize=(16, 5))
...
1. **ERP 代理偏差**：`bond.REPO_DAILY_PRICE` 是 1 天期国债逆回购利率，**不等于 10 年国债到期收益率**。本指标仅反映短端资金面松紧，与严格定义 ERP 有偏差。聚宽研究环境缺少直接的中长期国债收益率 API。如需更严谨的 ERP，建议自行准备 10 年国债收益率 CSV（如来自 Wind / 中债登）后在 cell 7 替换数据源。
...
5. **ERP 与官方源差异**：中金 / 万得的 ERP 通常用"沪深 300 盈利收益率 − 10 年国债收益率"，本模块用"中证全指盈利收益率 − 短端利率代理"，数值不可直接对比，仅供同源历史分位参考。
