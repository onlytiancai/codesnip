...
# 全局参数
AS_OF             = date.today()                       # 自动跟随聚宽环境当前日期（每次跑取最新）
...
# Step 5: 合并历史 + 增量（用完整交易日索引 reindex 补齐 NaN，避免"折线"）
if north_net:
    df_new_raw = pd.DataFrame(north_net).set_index('day').sort_index()
    # 关键：用完整日期索引 reindex，让缺失日填 NaN 而不是消失（解决图表"折线"问题）
    all_new_days = pd.DatetimeIndex(new_days)  # 增量期间的所有交易日（含 chunk=0 的空日）
    df_new = df_new_raw.reindex(all_new_days)
    ...
    nan_count = df_north_daily['北向资金日度净流入'].isna().sum()
    print(f"本次新增 {len(df_new_raw):,} 个有值日（reindex 后总 {len(df_north_daily):,} 日，{nan_count:,} 个 NaN）")
else:
