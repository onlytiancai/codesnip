...
9. **增量更新与 CSV 中间缓存**：cell 4 把 `df_north_daily` 持久化到 `outputs/tmp_north_daily.csv`（中间缓存，不是最终输出）。下次运行自动加载历史 CSV，只算 `history_max_date+1 ~ today` 的增量。增量首日的 `df_prev` 从 `history_max_date` 的快照重建（额外拉一次）。如果需要强制全量重算，删除 `outputs/tmp_north_daily.csv` 即可。
