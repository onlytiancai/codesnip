...
- -ss 10：从第 10 秒开始（输入侧 seek）
- -c copy：流复制，不重新编码，几秒完成

注意：

- -ss 放在 -i 前是快速但可能不精确的 seek（按关键帧对齐）。如果想在第 10 秒精确切，把 -ss 放到 -i 后（-i input -ss 10 -c copy），但会触发解码，耗时。
- 当前源是 H.264 + AAC，-c copy 安全，无需重编码。

如果想要重新编码保证精度：

    ffmpeg -i output/slide-1.mp4 -ss 10 -c:v libx264 -c:a aac output/slide-1-trimmed.mp4

想要精确且保留时间戳（慢但准）：

    ffmpeg -i output/slide-1.mp4 -ss 10 -c copy -avoid_negative_ts make_zero output/slide-1-trimmed.mp4