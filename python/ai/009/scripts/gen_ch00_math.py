...
def function_machine():
    ...
    ax.text(1.5, 0.7, "输入", ha="center", fontsize=11, color=MUTED)
    ...
    ax.text(5, 0.7, "函数（机器）", ha="center", fontsize=11, color=MUTED)
    ...
    ax.text(8.5, 0.7, "输出", ha="center", fontsize=11, color=MUTED)
    ...
    # 例子
    ax.text(5, 3.4, r"例子：$f(x) = 2x + 1$，输入 3 → 输出 7", ha="center", fontsize=12, color=TEXT, style="italic")

    save(fig, f"{OUT_DIR}/ch00_function_machine.png")


def coordinate_grid():
    """0.2 坐标系：横轴/纵轴 + 4 个标点"""
    fig, ax = new_figure(7, 6, 120, "坐标系（笛卡尔）")
    ...
    ax.set_xlabel("x (横轴)", fontsize=11, color=MUTED)
    ax.set_ylabel("y (纵轴)", fontsize=11, color=MUTED)
    ax.set_xticks(range(-5, 6))
    ax.set_yticks(range(-5, 6))
    save(fig, f"{OUT_DIR}/ch00_coordinate_grid.png")


def slope_tangent():
    """0.3 斜率：曲线 + 切线"""
    fig, ax = new_figure(7, 5, 120, r"斜率：$f(x) = x^2$ 在 $x=1.5$ 处的切线")
    ...
    ax.plot(x_tan, y_tan, "--", color=DANGER, lw=2, label=f"切线 (斜率={slope:.1f})")
    ax.plot(x0, y0, "o", color=DANGER, markersize=12, markeredgecolor="white", markeredgewidth=2)
    ax.annotate(f"切点 ({x0}, {y0:.1f})\n切线斜率 = {slope:.1f}", xy=(x0, y0),
    ...
    save(fig, f"{OUT_DIR}/ch00_slope_tangent.png")
...
def vector_personal():
    ...
    ax.text(5, 5.8, "小明的 4 维向量", ha="center", fontsize=14, fontweight="bold", color=TEXT)
    save(fig, f"{OUT_DIR}/ch00_vector_personal.png")
...
def matrix_multiplication():
    ...
    ax.text(7, 0.5, "形状：(2×2) × (2×1) = (2×1)\n左列数 = 右行数 → 匹配", ha="center",
            fontsize=10, color=MUTED, style="italic")

    save(fig, f"{OUT_DIR}/ch00_matrix_multiplication.png")
...
def chain_rule():
    ...
    # 标签
    ax.text(7, 3.5, "前向（Forward）", ha="center", fontsize=11, color=ACCENT, fontweight="bold")
    ax.text(7, 0.5, "反向（Backward）", ha="center", fontsize=11, color=DANGER, fontweight="bold")
    ...
    save(fig, f"{OUT_DIR}/ch00_chain_rule.png")


if __name__ == "__main__":
    print(f"== ch00_math（使用字体：{CHOSEN_FONT}）==")
    function_machine()
    coordinate_grid()
    slope_tangent()
    vector_personal()
