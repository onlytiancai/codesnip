...
onMounted(() => {
  ...
  if (!hasStoredAccounts()) {
    // 首次使用，显示欢迎信息
    successMessage.value = '欢迎使用2FA客户端！请设置密码并添加您的第一个2FA账户。';
  }
});
...
const authenticate = async () => {
  try {
    ...
    const storedAccounts = loadAccounts(password.value);
...
// 保存账户
const saveAccountList = async () => {
  try {
    isLoading.value = true;
    await new Promise(resolve => setTimeout(resolve, 300));
    saveAccounts(accounts.value, password.value);
...
const copyToClipboard = (code: string) => {
  navigator.clipboard.writeText(code).then(() => {
    ...
    successMessage.value = '验证码已复制到剪贴板！';
    setTimeout(() => {
      successMessage.value = '';
    }, 3000);
  }).catch(() => {
...
<template>
  <!-- 登录界面 -->
  <div v-if="!isAuthenticated" class="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 flex items-center justify-center p-4 font-sans">
  ...
  <!-- 主界面 -->
  <div v-else class="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 font-sans">
    <!-- 顶部导航栏 -->
