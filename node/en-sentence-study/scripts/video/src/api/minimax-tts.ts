...
//
// 用法：
//   import { synthesize, TTS_VOICES } from './api/minimax-tts';
//   const buf = await synthesize({ text: 'Hello', language: 'en' });
//   const buf = await synthesize({ text: '你好', language: 'zh', voiceId: 'female-shaonv' });
...
export type SynthesizeOptions = {
  ...
  voiceId?: string;       // 不传则按 language 选默认
  speed?: number;
  vol?: number;
  pitch?: number;
  model?: string;         // 默认 'speech-02-hd'
};
...
/**
 * 调用 MiniMax TTS，把文字合成成 MP3。
 * 返回 { buffer, durationMs, sampleRate }。
 */
...
/**
 * 嗅探 hex / base64 编码的音频字符串，返回 Buffer。
 */
export function decodeAudio(s: string): Buffer {
  // hex 特征：只含 0-9a-fA-F 且长度为偶数
