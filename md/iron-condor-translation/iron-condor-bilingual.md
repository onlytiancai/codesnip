...
| 项目 / Item | 数值 / Value |
| --- | --- |
| **Maximum Profit 最大收益** | **$200** |
| **Maximum Loss 最大亏损** | **$300** |
| **Potential Return 潜在回报率** | **66.67%** |
...
- **Monthly Condor** with Delta of **-16**（月度铁鹰，Delta = -16）
- **Weekly Condor** with Delta of **-12**（周度铁鹰，Delta = -12）
- **90 Day Condor** with Delta of **-10**（90 日铁鹰，Delta = -10）
...
- When the Term Structure is in **Contango**, markets are in a calm state and are behaving normally. → **Contango = 市场平静、运行正常**。
- When we shift to **Backwardation**, markets are in **panic mode**. → **Backwardation = 市场进入恐慌模式**。
...
1. **Use delta. i.e. Sell 10 delta or 15 delta** — 用 Delta 选：卖出 10 Delta 或 15 Delta 的期权。
2. **Use standard deviation. i.e. sell strikes 1 or 2 standard deviations away from the current price** — 用标准差选：卖出现价 ±1~2 个标准差之外的行权价。
3. **Use technical analysis** — 用技术分析选：参考支撑位、阻力位、趋势线等。
...
1. **Set the probability of success in your favour to ensure a statistical edge** — 设定对你有利的胜率，确保有**统计优势**。
2. **Only open Condors on underlying assets with a high IV Rank** — 只在 **IV Rank 高**的标的上建仓。
3. **Stick to the most liquid instruments (SPY, AAPL etc)** — 只挑**流动性最好**的标的（SPY、AAPL 等）。
4. **Be consistent in your process** — 流程要**保持一致**。
5. **Use appropriate position size to manage risk exposure** — 仓位**大小合适**地控制风险敞口。
6. **Use ETFs instead of individual stocks to mitigate earnings risk** — 优先 **ETF** 而非个股，避开**财报风险**。
7. **Keep an adequate cash buffer for adjustments (30-40%)** — 留出 **30-40%** 的**现金缓冲**用于调整。
8. **Avoid weekly options** — **远离周度期权**。
9. **Have a strict stop loss. Don't let losses blow out!** — 设定**严格的止损**，**别让亏损失控**！
...
1. **Use stop losses** — 设定**止损单**。
2. **Buy VIX call options** — 买入 **VIX Call**（押注波动率飙升）。
3. **Buy further out-of-the-money puts** — 买入**更虚值**的 Put（深度对冲）。
4. **Delta Hedge** — 做 **Delta 对冲**。
...
> * **到期日 / Expiration**：**2018-06-14***
> * **建仓日期**：2018-01-30（RUT 价格约 **1575**）*
> * **头寸 / Position**：3 份（手）*
