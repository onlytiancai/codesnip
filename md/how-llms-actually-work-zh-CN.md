...
> **Published**: Monday, June 01, 2026 - 26 mins
:::

::: zh
> **原文出处** ：[0xkato.xyz](https://www.0xkato.xyz/how-llms-actually-work/)
> **发表日期** ：2026 年 6 月 1 日（星期一）· 26 分钟阅读
...
![分词示意图：文本被切分为 token ID 序列](https://0xkato.xyz/assets/transformer-tokenization.png)
...
::: zh
你可以在嵌入上做算术运算，而且有时居然真的管用。著名例子是 `king − man + woman ≈ queen`。嵌入空间的几何结构承载着真实的语义结构，尽管没有人告诉模型要这样构建。
:::
...
::: zh
> **简明说明：因果掩码**
> 因果掩码隐藏未来的 token。它防止仅解码器语言模型在预测下一个 token 时向前看。
:::
...
::: zh
这是包括很多教程在内经常描述错误的部分。每个头并不是获得原始 token 向量的一个**字面意义上的**切片。每个头有自己的学习投影矩阵，将完整的 token 向量映射到自己的更小的 Q、K、V 向量。所以如果模型每个 token 有 4,096 个数字和 32 个头，每个头通常在 128 维空间中工作，但这 128 个数字是完整 4,096 的学习投影，而不是固定切片——是同一 token 的不同"视角"，而不是不同块。
:::
...
::: zh
这种存储记忆特性延伸出了一些有意思的应用。研究人员已经弄清楚如何在不重新训练的情况下直接编辑训练模型中的一些事实。 **ROME（Rank-One Model Editing）** 等方法可以**对特定 FFN 权重矩阵实施有针对性的低秩修改**，将"埃菲尔铁塔在巴黎"改为"埃菲尔铁塔在罗马"。然后模型倾向于生成与编辑后的关联一致的内容。
:::
...
::: zh
这就是这些看似不起眼的底层机制。没有残差连接，非常深的模型变得难训练得多。没有层归一化，残差流可能爆炸或崩溃。有了两者，你就可以堆叠出数百层深的模型。
:::
...
::: zh
> **简明说明：推测解码（Speculative Decoding）**
...
::: zh
那么 GPT、Claude、Gemini 和 LLaMA 之间实际有什么不同？公开细节各不相同，专有模型也不会公布每个架构选择。但从本文所覆盖的层面看，**它们都属于同一个 Transformer 家族的设计范畴**。
:::
...
Transformer 家族架构的趋同在机器学习史上并不常见。在该领域的大部分历史中，每个问题都有自己专门的网络：图像识别用一种，语言用另一种，音频用第三种。视觉与语言团队之间几乎不共享方法。

如今 Transformer 风格的模型已经出现在语言、视觉、音频以及多模态系统中。Transformer 几乎吸纳了整个领域的成果。

但这种局面可能改变。Mamba 和其他状态空间模型（State-Space Model，SSM）是有力的替代方案，尤其适合处理超长序列。混合架构也正在被探索。混合专家模型（Mixture-of-Experts，MoE）已经改变了"架构"在前沿模型中的含义——这种变化在五年前会被视为异端。

但本文介绍的核心机制（分词、嵌入、位置编码、注意力、前馈网络、残差流与归一化，以及下一个 token 预测）是经久不衰的部分。即使架构本身发生改变，这些仍是任何序列模型必须以某种形式解决的问题。

如果你读到了这里，你应该能读懂许多现代 Transformer 论文或模型卡片，并知道每一节在讲哪个部分。这正是本文的目标。

非常欢迎反馈。如果你对这些内容感兴趣，欢迎在 [X](https://x.com/0xkato) 上联系我。我很乐意结交新朋友。
:::

---

::: en
> **Source link**: [How LLMs Actually Work](https://www.0xkato.xyz/how-llms-actually-work/)
> **Translator**: AI-assisted translation + human review
